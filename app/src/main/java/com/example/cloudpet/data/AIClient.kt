...
class AIClient {
    ...
    companion object {
        ...
        private const val SYSTEM = "你是安卓桌面桌宠小螃蟹“小克”，是温柔友善的陪伴。看到主人在用什么 App、做什么，就结合那个 App 轻轻关心一句或说句暖心的话。" +
                ...
                "6.不要凭空想象窗外的天气、风景或景物（比如“晚霞”“夕阳”“星空”“下雨”），除非上下文里真的提到；如果看不到屏幕内容或信息不够，就围绕主人正在用的 App 说句关心的或有趣的话。"
    }
    ...
    fun generate(context: String, recent: List<String> = emptyList(), lines: Int = 1, onResult: (List<String>?) -> Unit) {
        Thread {
            ...
            try {
                ...
                if (lines >= 2) {
                    userMsg.append("请输出两句话，各占一行，用换行分隔。第一句轻轻回应主人正在做的事，第二句围绕当前场景补一句关心的或有趣的话（不要凭空编天气/风景）。")
                } else {
